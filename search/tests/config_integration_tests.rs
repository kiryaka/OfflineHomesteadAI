...
mod tests {
    ...
    fn test_prod_config_loading() {
        ...
        assert!(
            config.lancedb_search.nprobes >= 50,
    ...
    fn test_environment_loading() {
        ...
        assert!(
            dev_config.lancedb.num_partitions <= 1000,
            "Dev config should be loaded correctly"
        );

        // Test prod loading
        let prod_config = Config::load_for_env(Some("prod")).expect("Failed to load prod config");
        assert!(
            prod_config.lancedb.num_partitions >= 1000,
            "Prod config should be loaded correctly"
        );
    }
    ...
    fn test_parameter_scaling() {
        ...
        assert!(
            ...
            prod_config.lancedb.num_partitions,
            dev_config.lancedb.num_partitions
        );
        ...
        assert!(
            ...
            prod_config.lancedb_search.nprobes,
            dev_config.lancedb_search.nprobes
        );
        ...
        assert!(
            ...
            prod_config.lancedb_search.refine_factor,
            dev_config.lancedb_search.refine_factor
        );
    }
    ...
    fn test_config_validation() {
        ...
        assert!(
            invalid_dev_config.validate_for_env("dev").is_err(),
        ...
        assert!(
            invalid_prod_config.validate_for_env("prod").is_err(),
    ...
    fn test_performance_characteristics() {
        ...
        assert!(
            ...
            dev_vectors_per_partition,
            prod_vectors_per_partition
        );

        // Calculate search complexity
        let dev_search_complexity =
            dev_config.lancedb_search.nprobes * dev_config.lancedb_search.refine_factor;
        let prod_search_complexity =
            prod_config.lancedb_search.nprobes * prod_config.lancedb_search.refine_factor;
        ...
        assert!(
            ...
            dev_search_complexity,
            prod_search_complexity
        );
    }
    ...
    fn test_memory_characteristics() {
        ...
        assert_eq!(
            dev_memory_per_partition, prod_memory_per_partition,
