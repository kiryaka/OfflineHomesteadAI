...
fn main() -> anyhow::Result<()> {
    ...
    println!("    • num_partitions: {} (1,000-4,000 vectors per partition)", config.lancedb.num_partitions);
    println!("    • num_sub_vectors: {} ({} / {} = {} sub-vectors, SIMD optimized)", 
             config.lancedb.num_sub_vectors, 
             config.embedding.dimension,
             config.lancedb.num_sub_vectors,
             config.embedding.dimension / config.lancedb.num_sub_vectors);
    println!("    • metric: {} (optimal for text embeddings)", config.lancedb.metric);
    println!();
    
    println!("  🔍 Search Configuration (PQ + Re-ranking):");
    println!("    • nprobes: {} ({}% of partitions for good recall)", 
             config.lancedb_search.nprobes,
             (config.lancedb_search.nprobes * 100) / config.lancedb.num_partitions);
    println!("    • refine_factor: {}x (over-retrieval for flat re-ranking)", config.lancedb_search.refine_factor);
    println!("    • default_limit: {} (final results after re-ranking)", config.lancedb_search.default_limit);
    println!("    • max_limit: {} (maximum results allowed)", config.lancedb_search.max_limit);
    println!();
    ...
    let pq_compression_ratio = config.embedding.dimension as f32 / config.lancedb.num_sub_vectors as f32;
    ...
    println!("  • Vectors per partition: {} (optimal range: 1,000-4,000)", vectors_per_partition);
    println!("  • PQ compression ratio: {:.1}x", pq_compression_ratio);
    println!("  • Storage reduction: {:.1}%", storage_reduction);
    println!("  • Search probes: {} partitions ({}% coverage)", 
             config.lancedb_search.nprobes,
             (config.lancedb_search.nprobes * 100) / config.lancedb.num_partitions);
    println!();
    ...
    println!("  1. PQ Search: Retrieve top {} candidates using compressed vectors", 
             config.lancedb_search.default_limit * config.lancedb_search.refine_factor);
    println!("  2. Flat Re-ranking: Compute exact distances for {} candidates", 
             config.lancedb_search.default_limit * config.lancedb_search.refine_factor);
    println!("  3. Final Results: Return top {} most relevant documents", 
    ...
    println!("  • Space savings: {:.1}%", ((flat_storage_gb - pq_storage_gb) / flat_storage_gb) * 100.0);
    println!();
