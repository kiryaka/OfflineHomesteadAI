...
use tantivy::{
    ...
    doc,
    query::QueryParser,
    schema::{Facet, Schema, TextFieldIndexing, TextOptions, IndexRecordOption, FacetOptions, STRING, STORED, Value},
...
impl TantivySearchEngine {
    ...
    pub fn get_facet_counts(&self, query_text: &str) -> Result<Vec<(String, u64)>, anyhow::Error> {
        ...
        for (facet, count) in facet_counts.get(&Facet::root().to_string()) { facets.push((facet.to_string(), count)); }
        Ok(facets)
    }
}
